...
const ChatArea = ({
      ...
      <div className="flex-1 bg-white/6 border border-white/10 rounded-2xl flex-col items-center justify-center p-6 backdrop-blur-md shadow-2xl relative overflow-hidden hidden min-[1000px]:flex">
        ...
        <div className="flex flex-col items-center text-center max-w-sm px-4">
          ...
          <p className="text-gray-400 text-sm leading-relaxed">
            Select a contact to start chatting, or send a request to someone
            new.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="flex-1 bg-white/6 border border-white/10 rounded-2xl relative flex flex-col backdrop-blur-md shadow-2xl relative overflow-hidden w-full min-[1000px]:w-auto">
      {/* Header Bar */}
      <div className="p-4 border-b border-white/10 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-fuchsia-500 text-white text-sm font-bold flex items-center justify-center shrink-0 shadow-md">
            {(selectedContact.username || selectedContact.name || "U")
              .charAt(0)
              .toUpperCase()}
          </div>
                ...
                <React.Fragment key={message._id || index}>
                  ...
                  >
                    ...
                    >
                      <p>{message.text}</p>
                      <p
                        className={`text-[10px] mt-1 ${isOwnMessage ? "text-white/70 text-right" : "text-gray-400 text-left"}`}
                      >
                        {time}
                      </p>
                    </div>
      ...
      {/* Message Input Bar */}
      <div className="p-4 mb-3 pb-[env(safe-area-inset-bottom)] border-t border-white/10 shrink-0">
        <div className="flex items-center w-full bg-white/5 ring-2 ring-white/10 focus-within:ring-indigo-500/60 h-12 rounded-full px-4 gap-2 transition-all">
          <button
            className="cursor-pointer text-white/70 hover:text-white transition text-center"
            type="button"
            onClick={() => setShowPicker((prev) => !prev)}
          >
            <i className="fa-regular fa-face-smile-beam text-xl"></i>
          </button>
          {showPicker && (
            <div
              ref={pickerRef}
              className="absolute bottom-20 left-4 z-50 max-w-full sm:min-w-2xl max-w-md"
            >
              <EmojiPicker
                theme="dark"
                className="!bg-[#0c0d14] !border !border-white/10 !rounded-2xl shadow-2xl"
                style={{
                  "--epr-bg-color": "#0c0d14",
                  "--epr-category-label-bg-color": "#0c0d14",
                  "--epr-picker-border-color": "rgba(255, 255, 255, 0.1)",
                  "--epr-search-input-bg-color": "rgba(255, 255, 255, 0.08)",
                  "--epr-hover-bg-color": "rgba(255, 255, 255, 0.15)",
                }}
                width="100%"
